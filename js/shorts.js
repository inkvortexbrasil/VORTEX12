...
window.generateShortsInputs = async function() {
  ...
  // Guarda contra dupla execução e proteção global do ESC
  if (window.anyJobRunning) return;
  ...
  window.anyJobRunning = true; // PROTEÇÃO GLOBAL: bloqueia ESC durante geração de imagens Shorts
  btn.disabled = true;
  ...
  try {
    ...
    const geminiScenes = camp && Array.isArray(camp.geminiScenes) ? camp.geminiScenes : [];
    if (!camp || !camp.scenes || camp.scenes.length < 10 || geminiScenes.length < 5) {
      throw new Error("Minissérie precisa das 10 cenas GPT e das 5 cenas Flow + Gemini.");
    }
    
    // 1. Extrair os Prompts Reais
    const gptPrompts = camp.scenes.map(s => s.prompt);
    const geminiPrompts = geminiScenes.map(s => s.geminiMotion || s.prompt || '');

    if(statusBar) statusBar.style.width = '40%';
    if(statusPct) statusPct.innerText = '40%';
    if(statusLog) statusLog.innerText = `Enviando os prompts GPT e Flow + Gemini da Minissérie ${camp.number}... Isso pode levar até 1 minuto.`;
    ...
    window.anyJobRunning = false; // PROTEÇÃO GLOBAL: libera ESC mesmo em erro
...
window.assembleShorts = async function() {
  ...
  window.anyJobRunning = true; // PROTEÇÃO GLOBAL: bloqueia ESC durante montagem de Shorts
  btn.disabled = true;
